...
namespace MSFSPopoutPanelManager.TouchPanel.SimConnectAgent
{
    public class ActionProvider
    {
        ...
        public ActionProvider(SimConnector simConnector)
        {
        ...
        public void ExecAction(SimConnectActionData actionData)
        {
            ...
            if (_isSimConnected && actionData.Action != null)
            {
                ...
                {
                    TouchPanelLogger.ServerLog(e.Message, LogLevel.ERROR);
                }
            }
        }
        ...
        public void ArduinoInputHandler(object sender, EventArgs<ArduinoInputData> e)
        {
            if (_actionQueue.Count < MAX_ACTION_QUEUE_COMMAND && _currentSimConnectEncoderAction != null)
            {
                ...
                if (e.Value.Acceleration == 1)
                {
                    _actionQueue.Enqueue(commandAction);
                }
                else
                {
                    for (var a = 0; a < e.Value.Acceleration; a++)
                    {
                        _actionQueue.Enqueue(commandAction);
                    }
                }
